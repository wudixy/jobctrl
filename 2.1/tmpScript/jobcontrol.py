...
def getRepDBName():
    # 初始化DBNAME
    repdbpath = proPath() + '\\' + 'runtimedb'
    if not os.path.exists(repdbpath):
        os.mkdir(repdbpath)
    return repdbpath + '\\' + tconfig['TASKNAME'] + '.DB'
...
def getJobStatus(fname, jobid):
    ...
    cur = conn.cursor()
    try:
        sql = "SELECT status,msg FROM cur_jobstatus WHERE jobid='%s'" % (jobid)
        cur = cur.execute(sql)
        have = cur.fetchone()
        cur.close()
        conn.close()
...
def cfg_getJobStatus(jobid):
    jobcfg = getJobInfo(jobid, tconfig)
    st = getJobStatus(getRepDBName(), jobcfg['EXECUTE'])
    if st['st'] == 'exception':
        return {'st': 'exception', 'rcd': ''}
    else:
        return {'st': 'normal', 'rcd': {'status': st['st'], 'msg': st['msg']}}
...
def getReplicateTaskStatus(pdir, ddir, taskname):
    ...
    try:
        ...
        r = {'state': xx['task_status']["state"],
             "full_load_completed": xx['task_status']['full_load_completed'],
             'start_time': getArTime(xx['task_status']['start_time']),
             'stop_time': getArTime(xx['task_status']['stop_time']),
             }
    except Exception, e:
        logging.error(str(e))
    return r
...
def cfg_getReplicateTaskStatus(jobid):
    jbcfg = getJobInfo(jobid, tconfig)
    st = getReplicateTaskStatus(tconfig['ARCONFIG']['PRO_DIR'],
                                tconfig['ARCONFIG']['DATA_DIR'],
                                jbcfg['EXECUTE'])
    if st['state'] != 'OTHER':
...
def waitAttunityTask(pdir, ddir, taskname, waitsec, timeoutcnt):
    ...
    for i in range(timeoutcnt):
        ...
        if st['state'] == 'STOPPED' or st['state'] == 'OTHER':
            break
        time.sleep(waitsec)
    if st['state'] == 'STOPPED' and st['full_load_completed'] == 'true':
...
def select_Oracle_sql(tns, use, pwd, execute):
    """执行ORACLE的sql语句"""
    try:
...
def cfg_select_Oracle_sql(jobid):
    jobcfg = getJobInfo(jobid, tconfig)
    return select_Oracle_sql(jobcfg['CONNECTINFO']['TNS'],
...
def cfg_getCanRunJob(fname):
    ...
    for job in tconfig['TASKCONFIG']:
        # 只处理等待类型的作业
        # v2.1 return code changed dict
        if getJobStatus(fname, job['JOBID'])['st'] != 'WAIT':
            continue
        # 如果没有依赖
        if job['DEP'] == '':
            jobid = job['JOBID']
...
def cfg_runJobOnce(jobid):
    ...
    if jobcfg['JOBTYPE'] == 'WAIT_DB_FLAG':
        ...
        if jobcfg['CONNECTINFO']['TYPE'] == 'ORACLE':
            res = cfg_select_Oracle_sql(jobid)
        ...
        res = cfg_getJobStatus(jobid)
        st = checkSucessExpress(jobcfg, res)
    elif jobcfg['JOBTYPE'] == 'GET_AR_TASK_STATUS':
        res = cfg_getReplicateTaskStatus(jobid)
        st = checkSucessExpress(jobcfg, res)
    else:
        st = 'ERROR'
        logging.error('job:%s,type:%s is not support' %
...
def cfg_runJob(fname, jobid):
    ...
    for i in range(timeoutcnt):
        st, rcd = cfg_runJobOnce(jobid)
        if st == 'FINISH':
            break
        time.sleep(waitsec)

    if ('SENTMAIL' in jobcfg.keys()) and (jobcfg['SENTMAIL'] == 1):
        cfg_send_mail('%s is %s' % (jobid, st),
                      'msg is %s' % (formatMsg(rcd)))

    writeJobStatus(fname, jobid, enddt=getTime(),
                   status=st, msg=formatMsg(rcd))
